/* -------------------------------------------------------------------------- */
/*                Gallery merupakan modifikais dari Carousel 🤖               */
/* -------------------------------------------------------------------------- */

import React, { useState, useEffect } from "react";
import { Image } from 'antd';
import { IconArrowCarouselLeft, IconArrowCarouselRight } from "../../assets/images/index.tsx";
		...
		<div className="relative flex flex-col lg:flex-row h-full p-0 lg:px-4 lg:py-4">
			...
			<div className="relative w-full flex flex-col lg:flex-row pb-16 lg:pb-8">
				...
				<div className="w-full md:w-[80%] mx-auto">
					<Image src={items[currentIndex].image} alt="Gallery Image" className="object-cover px-4 pt-4 pb-0 md:p-6" />
				</div>
				{/* <div className="w-full lg:w-1/2 flex flex-col justify-center items-center lg:items-start text-center lg:text-start p-4">
					<div className={`${css.txtHeadline1} mb-6`}>{items[currentIndex].text_team}</div>
					<div className={`${css.txtBodyBold} mb-12`}>{items[currentIndex].text_winner}</div>
					<div className="relative">
						<img src={IconQuote} alt="Quote" className="absolute top-[-24px] right-[24px] z-10" />
						<div className={`${css.txtBody} glass-effect-1 rounded-[24px] p-8`}>{items[currentIndex].text_quote}</div>
					</div>
				</div> */}
